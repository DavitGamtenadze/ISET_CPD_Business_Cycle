#!/usr/bin/env python
"""
Comprehensive Quarterly Data Creation

Creates quarterly dataset with all requested columns from monthly data sources.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / 'comprehensive_quarterly_data.log'),
...
def load_comprehensive_monthly_data():
    """Load all monthly datasets needed for comprehensive quarterly data."""
    logger.info("Loading comprehensive monthly datasets...")
...
def create_comprehensive_quarterly_data(datasets):
    """Create comprehensive quarterly dataset with all requested columns."""
    logger.info("Creating comprehensive quarterly dataset...")
    ...
    if 'reer' in datasets and 'REER' not in str(base_data.columns):
        ...
        # Convert REER date to datetime to match base_data
    ...
    if 'neer' in datasets and 'NEER' not in str(base_data.columns):
        ...
        # Convert NEER date to datetime to match base_data
    ...
    if business_confidence_data is not None:
        # Merge business confidence data (left join to keep all quarterly data)
    ...
    for col in quarterly_data.columns:
        if col in desired_columns:
            ...
            # Check for partial matches with exchange rate patterns
...
def save_comprehensive_quarterly_data(df):
    """Save comprehensive quarterly data."""
    try:
        output_dir = project_root / 'data' / 'quarterly_data'
        output_dir.mkdir(exist_ok=True)
        
        # Only save Excel file (no CSV as requested)
        excel_path = output_dir / 'georgia_comprehensive_quarterly_data.xlsx'
        df.to_excel(excel_path, index=False)
        
        logger.info(f"Comprehensive quarterly data saved to {excel_path}")
        
    except Exception as e:
        logger.error(f"Error saving comprehensive quarterly data: {e}")
        raise


def main():
    """Main processing function."""
    logger.info("Starting comprehensive quarterly data creation...")
    
    try:
        # Load all monthly datasets
        datasets = load_comprehensive_monthly_data()
        
        if not datasets:
            logger.error("No datasets loaded")
            return
        
        # Create comprehensive quarterly data
        quarterly_data = create_comprehensive_quarterly_data(datasets)
        ...
        # Save the data
        save_comprehensive_quarterly_data(quarterly_data)
        
        logger.info("Quarterly data creation completed!")
